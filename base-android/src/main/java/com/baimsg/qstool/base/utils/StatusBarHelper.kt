...
/**
 * Create by Baimsg on 2022/2/28
 *
 **/

@IntDef(
    STATUS_BAR_TYPE_DEFAULT, STATUS_BAR_TYPE_MIUI, STATUS_BAR_TYPE_FLYME, STATUS_BAR_TYPE_ANDROID6
)
@Retention(AnnotationRetention.SOURCE)
private annotation class StatusBarType

private const val STATUS_BAR_TYPE_DEFAULT = 0 // 默认
private const val STATUS_BAR_TYPE_MIUI = 1 // 小米
private const val STATUS_BAR_TYPE_FLYME = 2 // 魅族
...
private fun isMIUICustomStatusBarLightModeImpl(): Boolean {
    return if (DeviceHelper.isMIUI9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
        true
    } else DeviceHelper.isMIUI5() || DeviceHelper.isMIUI6() || DeviceHelper.isMIUI7() || DeviceHelper.isMIUI8()
}
...
private fun Context.initStatusBarHeight() {
    ...
    if (field != null && obj != null) {
        try {
            ...
            sStatusBarHeight = resources.getDimensionPixelSize(id)
        } catch (t: Throwable) {
            t.printStackTrace()
        }
    }
