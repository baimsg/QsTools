...
fun Long.toCalendar(): Calendar = Calendar.getInstance().also {
...
/**
 * @param month 从 0 开始，作为 [Calendar.MONTH]。
 */
fun calendar(
    year: Int,
    month: Int,
...
infix fun Calendar.sameDate(other: Calendar) =
    sameYear(other) && sameMonth(other) && (day() == other.day())
...
infix operator fun Calendar.compareTo(that: Calendar): Int =
    (timeInMillis - that.timeInMillis).toInt()
